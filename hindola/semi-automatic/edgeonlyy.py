...
class Model(nn.Module):
	def __init__(self, input_height, input_width, input_channels):
		...
		self.layer_d4 = nn.Sequential(nn.Conv2d(128, 256, 3, padding=1, stride=2, bias=True), nn.BatchNorm2d(256),
									  nn.ReLU(), nn.Conv2d(256, 256, 3, padding=1, stride=1, bias=True))
		self.layer_d4s = nn.Conv2d(128, 256, 1, stride=2)
		...
		self.layer100 = nn.Sequential(nn.Conv2d(256, 16, 1, padding=0, bias=True), nn.ReLU())
		# self.layer101 = nn.Sequential(nn.Conv2d(128, 64, 1, padding=0, bias=True), nn.ReLU())


		self.layer9 = nn.Sequential(nn.Linear(3840, 1600, bias=True))

		self.layer200 = nn.AvgPool2d(8,30)
		self.layer201 = nn.Sequential(nn.Linear(16,6, bias = True), nn.Softmax(dim = 1))
	...
	def forward(self, input_img):
		...
		return edge_logits1, class_prob
