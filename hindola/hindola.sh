...
docker run -it -d --network host --name hindola -e DB_USER=root -e DB_PASS=root -e DB_NAME=annotation_web -e DB_HOST=localhost -e APP_HOST=${hostip} -e DATASET_LOC=${datasetdir} $(ls -la /dev | grep nvidia | sed 's/\s\s*/ /g' | cut -d ' ' -f 10 | while read -r line; do echo "--device /dev/$line:/dev/$line"; done | tr '\n' ' ') -v "${HINDOLA_SAVE_DIR}:/save" -v "${datasetdir}:/data" -p ${hostip}:10000:10000 hindola/local /bin/bash
